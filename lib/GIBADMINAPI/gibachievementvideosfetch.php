...
if ($_SERVER['REQUEST_METHOD'] === 'DELETE') {
    ...
    $sql = "DELETE FROM gibachievementvideos WHERE id = $videoId";
...
$selectQuery = "SELECT event_name, selectedDate, GROUP_CONCAT(id) as ids, GROUP_CONCAT(videos_name) as videos_names, GROUP_CONCAT(videos_path) as videos_paths, GROUP_CONCAT(thumbnail_path) as thumbnail_paths FROM gibachievementvideos GROUP BY event_name, selectedDate";
...
while ($row = $result->fetch_assoc()) {
    $videos[] = array(
        ...
        'videos' => array_map(function($id, $name, $path, $thumb) {
            return array(
                'id' => $id,
                'videos_name' => $name,
        ...
        }, explode(',', $row['ids']), explode(',', $row['videos_names']), explode(',', $row['videos_paths']), explode(',', $row['thumbnail_paths']))
    );
}
